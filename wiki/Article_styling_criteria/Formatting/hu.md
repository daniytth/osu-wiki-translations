...
::: alert-note
**Jegyzet:** Az írási követelmények tekintetében, lásd [Article style criteria/Writing](../Writing) részt.
:::

::: alert-notice
**Közlemény:** Ez a cikk az [RFC 2119](https://tools.ietf.org/html/rfc2119) szabványt használja a követelményszintek leírásához.
:::

## Nyelvek

Az alábbiakban felsoroljuk a wiki által megfelelően támogatott nyelvi beállításokat:

| Fájl Neve  | Nyelv Neve                   | Natív Írása        |
|:-----------|:-----------------------------|:-------------------|
...
::: alert-notice
**Közlemény**
A weboldal az olvasók számára a cikk választott nyelvű változatát jeleníti meg. Ha az nem áll rendelkezésre, akkor az angol nyelvű változatot jeleníti meg.
:::

## Előszó

A bevezető részt a fájl legelejére kell elhelyezni. Ez [YAML](https://en.wikipedia.org/wiki/YAML#Example) formátumban íródik, és a cikkre vonatkozó kiegészítő információkat tartalmaz. A szöveget felett és alatt három kötőjellel (`---`) kell keretezni, és a cím felett egy üres sorral kell elválasztani.

### Cikkek amiben kell segíteni

::: alert-notice
**Közlemény**
Kerüljék az ezzel a címkével ellátott angol cikkek fordítását. Ezen felül ezt a címkét akkor kell hozzáadni, ha a fordításon még javításra van szükség.
:::

A `needs_cleanup` címke hozzáadható azokhoz a cikkekhez, amelyek átírásra vagy formázási segítségre szorulnak. Erre a célra GitHubon is lehet hibajelentést nyitni. A címkét az alábbiak szerint kell megadni:

```yaml
needs_cleanup: true
```

Ha ezt a címkét hozzáadod egy cikkhez, akkor a [megjegyzéseket](#comments) is hozzá kell adnod, hogy elmagyarázd, mit kell tenni a címke eltávolításához.

### Régi Cikkek

::: alert-notice
**Közlemény**
Kerüljék az ezzel a címkével ellátott angol cikkek fordítását. Ha az angol cikk rendelkezik ezzel a címkével, akkor a fordításnak is tartalmaznia kell ezt a címkét.
:::

Az angol nyelvű cikkek elavulhatnak, ha a bennük szereplő tartalom félrevezető vagy már nem releváns. Ezeket „outdated” címkével kell ellátni, amelyet az alábbiak szerint kell megadni:

```yaml
outdated: true
```

Amikor ezt a címkét hozzáadod egy cikkhez, a [megjegyzéseket](#comments) is hozzá kell adnod, hogy elmagyarázd, mit kell frissíteni a címke eltávolításához.

### Régi fordítások

Az elavult fordítású cikkeknél az `outdated_translation` címkét kell használni, ha az angol változat frissül, kivéve azokat az eseteket, amikor a cikk jelentését nem befolyásoló kisebb átfogalmazások, nyelvtani javítások és hasonló módosításokról van szó.

```yaml
outdated_translation: true
```

A fordítások elavulásakor azokhoz egy `outdated_since` címkét is hozzá kell rendelni, amely az angol változat frissítésének első commitjára utal.
